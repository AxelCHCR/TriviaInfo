...
def topJugadores():
    ...
    for i in range(0, len(datosOrdenados)):
        print(
            f"{i+1}. {datosOrdenados[i][0]} Cantidad de juegos: {datosOrdenados[i][1]}")
...
def topPreguntasFalladas():
    ...
    for i in range(0, len(datosOrdenados)):
        print(
            f"{i+1}. {datosOrdenados[i][0]} Cantidad de fallos: {datosOrdenados[i][1]}")
...
def iniciarJuego():
    ...
    while contadorRondas < 30:
        ...
        if respuesta == preguntas[seccion][pregunta][1]:
            ...
            addRegistroJugadasPorJugador(
                datosPreguntasAcertadas, datosJugadores[turno][0])
            guardarPreguntaFallada(
                "preguntasAcertadas.dat", datosPreguntasAcertadas)

        else:
            print("Respuesta incorrecta")
            falladas = cargarDatos("preguntasFalladas.dat")
            falladasCargadas = addPreguntasFalladas(
                preguntas[seccion][pregunta][0], falladas)
            guardarPreguntaFallada("preguntasFalladas.dat", falladasCargadas)
            datosFallosPorJugador = cargarDatos("fallosPorJugador.dat")
            addRegistroJugadasPorJugador(
                datosFallosPorJugador, datosJugadores[turno][0])
            guardarPreguntaFallada(
                "fallosPorJugador.dat", datosFallosPorJugador)
        turno += 1
    if tipoJuego:
    ...
    for i in range(0, cantJugadores):
        ...
        print(
            f"{datosJugadores[i][0]} acertó {datosJugadores[i][2]} preguntas ({(datosJugadores[i][2]/preguntasPorJugador)*100}%)")
    for i in range(0, len(datosJugadores)):
        datosCargados = cargarDatos("juegosDeUsuarios.dat")
        datoActualizado = sumarJuegoAJugador(
            datosJugadores[i][1], datosCargados)
